...
function generateCert(config) {
    ...
    if (config.android) {
        ...
        if (fs.existsSync(path.join(loc, certName))) {
            return console.error('安卓版证书已存在，无需再次生成', 'xproxy cert');
        }
        ...
        var opensslConfig = path.join(__dirname, 'openssl.cnf');
        ...
        if (isWin) {
            ...
            execSync('openssl req -new -x509 -key server.key -out xproxy_ca.crt -days 3650 -config ' + openCnfPath, {cwd: loc});
            execSync('openssl req -new -key server.key -out server.csr -config ' + openCnfPath, {cwd: loc});
            execSync('openssl x509 -req -days 3650 -in server.csr -CA xproxy_ca.crt -CAkey server.key -CAcreateserial -out server.crt -extfile ' + openCnfPath, {cwd: loc});
        } else {
            // 生成根证书
            execSync('openssl genrsa -out xproxy_ca.key 2048', {cwd: loc});
            execSync('openssl req -new -out xproxy_ca.csr -key xproxy_ca.key -config ' + openCnfPath, {cwd: loc});
            execSync('openssl x509 -req -days 3650 -sha256 -in xproxy_ca.csr -signkey xproxy_ca.key -out xproxy_ca.crt', {cwd: loc});
            // 生成server证书
            execSync('openssl genrsa -out server.key 2048', {cwd: loc});
            execSync('openssl req -new -out server.csr -key server.key -config ' + openCnfPath, {cwd: loc});
            execSync('openssl x509 -req -days 3650 -sha256 -CA xproxy_ca.crt -CAkey xproxy_ca.key -CAcreateserial -in server.csr -out server.crt -extensions v3_req -extfile ' + openCnfPath, {cwd: loc});
        }
